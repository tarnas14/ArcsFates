...
def _card_src(docs_path, name):
    # Page path is namespaced by top section (base/campaign) so same-named cards in
    # different sections don't collide. Keep in sync with hooks.py.
    section = docs_path.split("/", 1)[0].lower()
    return f"cards/{section}/{_slug(name)}.md"


def _page_markdown(docs_path, meta):
    name = meta.get("name") or posixpath.splitext(posixpath.basename(docs_path))[0]
    # Card pages live two directories deep (cards/<section>/<slug>.md), so images
    # resolve via "../../".
    lines = [f"# {name}", ""]
    if meta.get("type"):
        lines += [f"*{meta['type']}*", ""]
...
for _docs_path, _meta in _manifest.items():
    ...
    _out = _card_src(_docs_path, _name)
